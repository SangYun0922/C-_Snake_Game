...
void SnakeGame::StartWindow()
{
  ...
  init_pair(1, COLOR_RED, COLOR_RED); //cobra
  init_pair(2, COLOR_CYAN, COLOR_CYAN); //hole
...
void SnakeGame::RandGate()
{
  ...
  if(count3 == 0)
  {
    while(1)
    {
      ...
      gate2.y=rand()%21;
      if(((gate1.x!=gate2.x)||(gate1.y!=gate2.y))
       && !IsBody(gate1.y, gate1.x) && !IsBody(gate2.y, gate2.x))// && wall[gate1.y][gate1.x]==1 && wall[gate2.y][gate2.x]==1)
      {
        break;
      }
    }
...
bool SnakeGame::CheckMission()
{
  if(stage==1)
  {
    if(max_length>=3 && num_growth>=1 && num_poison>=1 && num_gate>=1)
    {
      stage++;
      countCobra = 0;
      return TRUE;
